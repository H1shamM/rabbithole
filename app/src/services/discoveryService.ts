...
export class DiscoveryService {
  ...
  async getRecommendations(userId: string, limit: number): Promise<StumbleAsset[]> {
  ...
  async stumble(category: string, history: string[], userId: string): Promise<StumbleAsset> {
    ...
    if (assets.length < 5) {
      console.log(`Only ${assets.length} assets in DB, fetching from live sources...`);
    ...
    const availableAssets = assets.filter((a: StumbleAsset) => !history.includes(a.id));
    ...
    const totalWeight = weightedAssets.reduce((sum: number, item: { weight: number }) => sum + item.weight, 0);
  ...
  private async fetchFromLiveSources(category: string): Promise<StumbleAsset | null> {
  ...
  async rate(assetId: string, isPositive: boolean, userId: string): Promise<void> {
    ...
    await this.storage.updateUserPreference(userId, 'category', asset.category, isPositive ? 1 : -1);
    await this.storage.updateUserPreference(userId, 'source', asset.source, isPositive ? 1 : -1);
  }
