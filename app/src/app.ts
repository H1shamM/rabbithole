...
export async function createApp() {
  ...
  const discoveryController = new DiscoveryController(discoveryService, storage);
  ...
  // OAuth routes
  if (settings.google) {
    v1Router.get('/auth/google', passport.authenticate('google', { scope: ['profile', 'email'] }));
  ...
  if (settings.github) {
    v1Router.get('/auth/github', passport.authenticate('github', { scope: ['user:email'] }));
  ...
  v1Router.post('/preferences', authenticateJWT, discoveryController.updatePreferences);
  ...
  v1Router.delete('/favorites/:id', authenticateJWT, discoveryController.removeFavorite);
  ...
  v1Router.get('/submissions', authenticateJWT, submissionController.getAllSubmissions);
