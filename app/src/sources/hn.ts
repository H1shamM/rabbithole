...
export class HackerNewsSource implements ContentFetcher {
  private readonly TOP_STORIES_URL =
    "https://hacker-news.firebaseio.com/v0/topstories.json";
  private readonly ITEM_URL_BASE =
  ...
  async fetchStumble(category: string): Promise<StumbleAsset> {
    try {
      const topStoriesRes = await fetchWithTimeout(
      ...
      if (!topStoriesRes.ok) {
        throw new Error(
          `HN Top Stories API error: ${topStoriesRes.statusText}`,
        );
      }
      const storyIds: number[] = await topStoriesRes.json();
      const randomId =
        storyIds[Math.floor(Math.random() * Math.min(storyIds.length, 50))];

      const itemRes = await fetchWithTimeout(
        `${this.ITEM_URL_BASE}${randomId}.json`,
        {},
        5000,
      );
      if (!itemRes.ok) {
