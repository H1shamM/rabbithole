...
export class SqliteAdapter implements IStoragePort {
  ...
  private migrate(): void {
    ...
    if (!userCols.includes('display_name')) this.db.exec('ALTER TABLE users ADD COLUMN display_name TEXT');
    if (!userCols.includes('avatar_url')) this.db.exec('ALTER TABLE users ADD COLUMN avatar_url TEXT');
    if (!userCols.includes('provider')) this.db.exec("ALTER TABLE users ADD COLUMN provider TEXT DEFAULT 'local'");
    const ratingsCols = (this.db.prepare('PRAGMA table_info(ratings)').all() as { name: string }[]).map(c => c.name);
  ...
  async saveAsset(asset: StumbleAsset): Promise<void> {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      asset.id, asset.url, asset.title, asset.description || null,
      asset.source, asset.category, asset.rating,
  ...
  // Rating & History
  async saveRating(user_id: string, asset_id: string, rating: 'like' | 'dislike'): Promise<void> {
    this.db.prepare(`
  ...
  // User preferences
  async updateUserPreference(user_id: string, type: 'category' | 'source', name: string, delta: number): Promise<void> {
    this.db.prepare(`
  ...
  async saveUser(user: User): Promise<void> {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      user.id, user.email, user.password_hash || null,
      user.display_name || null, user.avatar_url || null,
      user.provider, user.provider_id || null,
  ...
  async saveSubmission(submission: Submission): Promise<void> {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(submission.id, submission.user_id, submission.url, submission.title, submission.status, submission.created_at.toISOString());
  }
  ...
  // Recommendations
  async getRecommendations(user_id: string, limit: number): Promise<StumbleAsset[]> {
    const rows = this.db.prepare(`
      SELECT a.*
      FROM assets a
